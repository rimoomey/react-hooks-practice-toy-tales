...
- _When our application loads_, make a GET request to `/toys` to fetch the toy
  ...
  component, your next job is to render the `ToyCard` components on the page. done
...
- _When the `ToyForm` is submitted_, make a POST request to `/toys` to save a
  ...
  flow, think about how to render a new `ToyCard` for the toy that you created. done
...
- _When the `Donate to Goodwill` button is clicked_, make a DELETE request to
  ...
  server. The `ToyCard` that you clicked on should also be removed from the DOM. done
...
- _When the like button is clicked_, make a PATCH request to `/toys/:id` with
  ...
  should also increase the number of likes on the DOM. done
